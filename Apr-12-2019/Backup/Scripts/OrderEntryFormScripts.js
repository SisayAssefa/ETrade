...
function SelfTradeToggle() {
    ...
    var _client = $.trim($("#_drpClientId").select2("val"));
    var _symbol = $.trim($("#_drpSymbol").select2("val"));
    var _warehouse = $.trim($("#_drpWarehouse").select2("val"));
    var _productionYear = $.trim($("#_drpProductionYear").select2("val"));
    ...
    if ($("#chkIsSelfTrade").is(":checked") == true) {
        ...
        $("#_drpSession, #_drpClientId, #_drpSymbol, #_drpWarehouse, #_drpProductionYear").select2("val", "");
        $("#_drpClientId").select2("enable", false);
        ...
        $("#_drpClientId, #_drpSession, #_drpSymbol, #_drpWarehouse, #_drpProductionYear").select2("enable", true);
        $("#_drpSession, #_drpClientId, #_drpSymbol, #_drpWarehouse, #_drpProductionYear").select2("val", "");
        ClearCombo(combosToBeCleared);
        AddEmptyOptionToCombo(combosToBeCleared);
        //when the selftrade checkbox is toggled - reset the IF selection, if any
...
function PopulateSymbolsByMemberClientId(memberClientId) {
    ...
    if (clientId == "") {
        ...
        $("#_drpSymbol, #_drpWarehouse, #_drpProductionYear").select2("val", "");
    }
...
function InitializeOrderEntry() {
    try {
        ...
        var _userNumber = _selfTrade ? userInformation.MemberInfo.IDNO : $("#_drpClientId").select2("data").text;
...
function InitializeInstructionEntry() {
    try {
        ...
        var _userNumber = $("#_drpClientId").select2("data").text;
        var _clientId = $("#_drpClientId").val();
        var _warehouseId = $("#_drpWarehouse").val();
        var _productionYear = $("#_drpProductionYear").val();
        var _validity = $('input:radio[name=optionValidity]:checked').val();
        ...
        var _isIF = $('#chkIsIf').is(":checked");
        var _WRId = _isIF ? "00000000-0000-0000-0000-000000000000" : null;//TODO: Remove Hard Code, Use dropdown or Warehouse receipt string to resolve
        var _tradingCenterId = 1;
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data == true) {
                ...
                $('#busyImageSign').removeClass("alert alert-success");
                $('#busyImageSign').addClass("alert alert-error");
                $('#busyImageSign').append('  Failed to submit your' + msgAddOn + 'Instruction. Please try again later.');
                return false;
            }
            else {
                alert("Failed to continue. Please contact the administrator.");
        ...
        error: function (e) {
            $('#busyImageSign').html('<img src="Images/dialog_warning.png" />');
            $('#busyImageSign').addClass("alert alert-error");
            $('#busyImageSign').append('  Failed to save Instruction, Please try again later.');
        }
    });
}
...
function RessetOrderEntryForm() {
    $("#_drpClientId").select2("enable", true);
    $("#_drpSymbol").select2("enable", true);
    $("#_drpWarehouse").select2("enable", true);
    $("#_drpProductionYear").select2("enable", true);
    $("#_drpSession").select2("enable", true);
    $("#_drpClientId, #_drpSymbol, #_drpWarehouse, #_drpProductionYear, #_drpSession, #_drpWarehouseReceipts").select2("val", "");
...
function PopulateSymbolsByClient(clientId) {
    ...
    if (_symbolLength >= 0 || _warehouseLength >= 0 || _productionYearLength >= 0) {
            ...
            (_symbolLength == 0 || _warehouseLength == 0 || _productionYearLength == 0))//Not from table row data / New data entry
        {
            ClearCombo(combosToBeCleared);
            AddEmptyOptionToCombo(combosToBeCleared);
            $("#_drpSession, #_drpSymbol, #_drpWarehouse, #_drpProductionYear, #_drpWarehouseReceipts").select2("val", "");
...
function PopulateWarehousesByClientSymbol(clientId, symbolId) {
    ...
    if (symbolId == "" || ($("#chkIsSelfTrade").is(":checked") == false && clientId == "")) {
        ...
        ClearCombo(combosToBeCleared);
        AddEmptyOptionToCombo(combosToBeCleared);
        $("#_drpWarehouse, #_drpProductionYear, #_drpWarehouseReceipts").select2("val", "");
        $('#spnRangeHolder').html('');
        return;
    }
    var serviceUrl;
    ...
    if (selectedTab == 0) {
        ...
        serviceUrl = serviceUrlHostName + "Lookup/UIBindingService.svc/GetWarehousesBySymbol";
...
function PopulatePYearByClientSymbolWarehouse(clientId, symbolId, warehouse) {
    ...
    if (symbolId == "" || warehouse == "" || ($("#chkIsSelfTrade").is(":checked") == false && clientId == "")) {
        ...
        ClearCombo(combosToBeCleared);
        AddEmptyOptionToCombo(combosToBeCleared);
        $("#_drpProductionYear, #_drpWarehouseReceipts").select2("val", "");
        $('#spnRangeHolder').html('');
        return;
    }
    var serviceUrl;
...
function PopulateSymbolsBySessionId(_sessionId) {
    ...
    if ($("#chkIsSelfTrade").is(":checked") == false && $.trim($("#_drpClientId").select2("val")) == "") {
        $("#_drpSession").select2("val", "");
        return;
    }
...
function PopulateOddLotWarehouseReceipts() {
    ...
    var _clientId = $("#_drpClientId").select2("val");
    if ($.trim(_clientId) == "")
        _clientId = userInformation.MemberInfo.MemberId;
    var _symbol = $("#_drpSymbol").select2("val");
    var _warehouse = $("#_drpWarehouse").select2("val");
    var _year = $("#_drpProductionYear").select2("val");
    if ($.trim(_year) == "") {
        var combosToBeCleared = new Array("_drpWarehouseReceipts");
        ClearCombo(combosToBeCleared);
        AddEmptyOptionToCombo(combosToBeCleared);
        $("#_drpWarehouseReceipts").select2("val", "");
        return;
    }
...
function EmptyControls() {
    ...
    ClearCombo(combosToBeCleared);
    AddEmptyOptionToCombo(combosToBeCleared);
    $("#_drpClientId, #_drpSession, #_drpSymbol, #_drpWarehouse, #_drpProductionYear", "#_drpWarehouseReceipts").select2("val", "");
}
...
function CreateTicketConfirmationWindow() {
    InitializeTicketConfirmationData();
...
function InitializeTicketConfirmationData() {
    ...
    if (!isSelfTrade && (userInformation != undefined && (userInformation.UserTypeId == 2/*Client*/ || userInformation.UserTypeId == 6/*STP*/ || userInformation.UserTypeId == 5/*Rep*/))) {
        traderIdNo = $("#_drpClientId").select2("data").text;
    ...
    $("#spnSymbol").html($("#_drpSymbol").select2("data").text);
    $("#spnWarehouse").html($("#_drpWarehouse").select2("data").text);
    $("#spnYear").html($("#_drpProductionYear").select2("data").text);
    $("#spnQuantity").html($("#_txtQuantity").val());
    $("#spnWarehouseReceipt").html($("#_drpWarehouseReceipts").select2("data").text);
    ...
    if (!isSelfTrade) {//it is a client order - by a representative
        ...
        confirmationMessage = confirmationMessage.replace('#symbol', $("#_drpSymbol").select2("data").text);
        confirmationMessage = confirmationMessage.replace('#qty', $("#_txtQuantity").val());
        confirmationMessage = confirmationMessage.replace('#price', $("#_txtLimitPrice").val());
        $('#lblClientOrderConfirmationMessage').text(confirmationMessage);
        $('#lblClientOrderConfirmationMessage').show();
    } else {//it is a self order - so hide the message
        $('#lblClientOrderConfirmationMessage').hide();
    }
}
...
function FillQuantityFromWarehouseReceipt() {
    var _val = $("#_drpWarehouseReceipts").select2("val");
    if ($.trim(_val) == "") {
        ...
        var _value = $("#_drpWarehouseReceipts").select2("data").text;
...
$(function () {
    ...
    $(document).keyup(function (e) {
        ...
        if (e.keyCode == 13) { $("#rightMasterContent_OrderEntryFormUC_btnSubmitLang").click(); }   // enter
    ...
    $(document).click(function () { $('#editOrderPrice, #editOrderQty').tooltip("close"); });//If there is unclosed tooltip it is closed when the document is clicked    
});