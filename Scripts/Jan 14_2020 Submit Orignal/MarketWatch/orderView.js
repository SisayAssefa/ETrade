    ...
    j.ajax({
        ...
        success: function (data) {
            ...
            $.each(data.GetExecutedTradesResult, function () {
                ...
                html += '<td>' + (this.ExcutedQuantity * LotInQuintal).toFixed(4) + '</td>';
...
//Newly added function get hold sessions by Biniam and saron 05/03/2019
function IsSessionPaused() {
    sessionPaused = false;
    var url = serviceUrlHostName + 'SessionManager/UISessionServiceBindings.svc/GetWorkingSessions ';
        ...
        success: function (data) {
            if (data != null) {
                $.each(data, function (idx) {
                    if (this.OnlineStatus == 'Hold') {
                        sessionPaused = true;
                        //Orignal code to disable submit button
                        //$("#btnSubmitLang").attr("disabled", true);
                        //$("#btnMultiEdit").attr("disabled", true);
                        //End of orignal code
                    }
                });
    ...
    //Orignal code to freeze market watch, pengind orders
    //setTimeout(IsSessionPaused, 5000);
    //End of orignal code
    //--------------------------------
    }
...
function BindPendingOrders() {
    ...
    if (sessionPaused) {
        ...
        ///////////
        $("#btnMultiEdit").attr("disabled", false);
        $("#btnSubmitLang").attr("disabled", false);
        //$("#btnMultiEdit").show();
        //$("#btnSubmitLang").show();
            if (loadingPendings)
            ...
            j.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(data.GetPendingOrdersResult, function () {
                        ...
                        html += '<td Id="dataItemQty' + resultCount + '" >' + (qty * LotInQuintal).toFixed(4) + '</td>';
    ...
    j.ajax({
        ...
        success: function (data) {
            ...
            $.each(data.GetCancelledOrdersResult, function () {
                ...
                html += '<td>' + this.ProductionYear + '</td>';
                html += '<td>' + buyOrSellStyled + '</td>';
                html += '<td>' + (_remainingQ * LotInQuintal).toFixed(4) + '</td>';
                html += '<td>' + this.Price + '</td>';
                html += '<td class="dataItem" title="' + DayMonthYear + '" >' + HourMinSec + '</td>';
                html += '<td>' + orderOwner + '</td>';
                html += '<td>' + this.RepIDNo + '</td>';
...
function BindAllAtOnce() {
    ...
    setInterval(function () {
        ...
        if (SelectedTab === 2) {
           // if (isTherEdit == false && !isThereMultiEdit) {
    ...
    j.ajax({
        ...
        success: function (data) {
            ...
            $.each(data.GetOrderChangeSetHistoryByOrderIdResult, function () {
                ...
                html += '<td >' + this.Price + '</td>';
                html += '<td >' + (this.Quantity * LotInQuintal).toFixed(4) + '</td>';
...
function EditPendingOrder(evt, Id, rowNumber, isMktOrder) {
    ...
    IsSessionPaused();
    if (sessionPaused) {
        alert("Order can't be editted because session is paused from back office!");
        return;
    }
    else {
        if (evt.altKey) {
        ...
        //added by saron
        if (ItemQty <= 99) {
            var htmlInput = '<input type="text" class="inputEditPending" disabled id="editOrderQty" value="' + ItemQty.trim() + '" onclick="ValidatePriceRange(this)"></input><span visible="true" style="Color:Red"  id="errorEditOrderQty"></span>';
        ...
        $('#editOrderPrice, #editOrderQty').on('paste contextmenu dragstart dragover drop', function (e) {
            e.preventDefault();
        });
        $('#editOrderPrice, #editOrderQty').on('keypress keyup blur', function (e) {
            if (this.id == 'editOrderQty')
                KeyPressValidateQuantity(this, e);
            else if (this.id == 'editOrderPrice')
                KeyPressValidatePrice(this, e);
        });
        isTherEdit = true;
    }
}
...
function KeyPressValidateQuantity(elem, evt) {
    ...
    if (!$.isNumeric(charTyped) && !(evt.which == 8 || evt.which == 0 || evt.which == 46 || evt.which == 37 || evt.which == 39 || evt.which == 36 || evt.which == 35)) {
        ...
        if ($.isNumeric(charTyped)) {
            ...
            var allowedCharacters = decimalIndex == -1 ? 4 : 5 + decimalIndex;// decimalIndex == -1 || IsSellOrder == false ? 4 : 5 + decimalIndex;//maximum quantity could be 99 ***** changed to 4 after lot to quintal changes
            ...
            if (newText > MaximimOrderQtyPerTicket)
                evt.preventDefault();
            ...
            if ((charLength == 1 && newText.indexOf('.') == 0)) evt.preventDefault();
            if (evt.which == 8/*Backspace*/ || evt.which == 46/*Delete*/) //If backspace deletes the dot (.) and the left out number in text box exceeds maximum limit, then it clears the text box
            {
                ...
                if (currVal > MaximimOrderQtyPerTicket) {
                    $(elem).val(OrderOldQuantity);
                    return;
                }
            }
            if (evt.type == 'blur') {
...
function KeyPressValidatePrice(elem, evt) {
    ...
    if (!$.isNumeric(charTyped) && !(evt.which == 8 || evt.which == 0 || evt.which == 46 || evt.which == 37 || evt.which == 39 || evt.which == 36 || evt.which == 35)) {
        ...
        var x = $(elem).val();
        var newText = [x.slice(0, pos), charTyped, x.slice(pos)].join('');
        var totalLength = newText.length; //(charExisted + '' + charTyped).length;
        var numberEntered = parseFloat(newText); //(charExisted.toString() + charTyped.toString());
    ...
    j.ajax({
        ...
        success: function (data) {
            ...
            $.each(data, function () {
                ...
                html += '<tr><td>Rep ID No:</td><td>' + this.RepIDNo + "</td></tr>";
                html += '<tr><td>Buy/Sell:</td><td>' + this.BuyOrSell + '&nbsp &nbsp' + (this.OrderType == null ? "" : this.OrderType) + '</td></tr>';
                html += '<tr><td>Order Qty:</td><td> Remaining:(' + (remQty == null ? "" : '' + (remQty * LotInQuintal).toFixed(4)) + ') &nbsp;  &nbsp;  Original:(' + (orginal * LotInQuintal).toFixed(4) + ') &nbsp &nbsp' + this.FillTypeName + '</td></tr>';
                html += '<tr><td>Order Price:</td><td>' + (this.Price == null ? "" : this.Price + '&nbsp;-&nbsp;') + this.OrderType + '</td></tr>';
...
function IsEditedPriceValid(mktOrder) {
    ...
    OrderOldQuantity = parseFloat(OrderOldQuantity);
    ...
    //    return;
    //}
    if (OrderOldPrice == editedPrice && OrderOldQuantity == parseFloat(editedQuantity)) {
    ...
    if (OrderOldQuantity != parseFloat(editedQuantity)) {
        ...
        else if (parseFloat(editedQuantity) < 1) {
            $("#traderUIPrompt").showEcxPrompt({ title: "Wrong Input", message: "You entered incorrect quantity, Zero is not allowed.", type: "error" });
            $("#editOrderQty").focus();
            return false;
        }
    }
    if (OrderOldPrice != parseInt(editedPrice)) {
...
function SubmitEdit(Id, rowNumber, isMarketOrder) {
    ...
    var intOrderQtyTemp = parseFloat(orderQtyTemp);
    var intRemainingForContract = parseFloat(RemainingForContract);
    var maxNewQuantityAllowed = parseFloat(OrderOldQuantity + intRemainingForContract);
    ...
    if (IsSellOrder) {
        if (intOrderQtyTemp < 50 && maxNewQuantityAllowed >= 100) {

            $("#traderUIPrompt").showEcxPrompt({ title: "Invalid Quantity ", message: "Quantity should be 50 or above to update your sell order.", type: "warning" });
            return;
        }
    }
    if (maxNewQuantityAllowed <= 99 && maxNewQuantityAllowed != intOrderQtyTemp) {
        if (IsSellOrder) {
            $("#traderUIPrompt").showEcxPrompt({ title: "Invalid Quantity ", message: "Quantity should not be less than the available quantity.", type: "warning" });
            return;
        }
    }
    if (intOrderQtyTemp < 50) {
        if (!IsSellOrder) {
            $("#traderUIPrompt").showEcxPrompt({ title: "Invalid Quantity ", message: "Quantity should be 50 or above to update your sell order.", type: "warning" });
            return;
        }
...
function InitializeOrderForEdit(orderIdNo, updatedQty, updatedPrice, uID) {
    var _initializedObject = {
        OrderId: orderIdNo,
        Quantity: updatedQty / LotInQuintal,
        Price: updatedPrice,
        CurrentUserId: uID
    }
...
$(function () {
    ...
    //Newly added function get hold sessions by Biniam and saron

    //Orignal Code for paused market watch
    //IsSessionPaused();
    //End of origanl code 
