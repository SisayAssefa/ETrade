...
function BindPendingOrders() {
    ...
    if (!userInformation) return;//if there is no user infromation, do nothing
    SelectedTab = 2;
            ...
            $.each(data.GetPendingOrdersResult, function () {
                ...
                if (userTypeId != 2) {
                    ...
                    if (this.IsPartiallyExecuted)
                        ...
                        disableEdit = 'title="Edit this order..."';
                    if (isInstruction != true) {

                        html += '<button type="button" class="btn btn-link" ' + disableEdit + '  onclick="GetExistingValuesBeforeEdit(this); EditPendingOrder(' + id + ',' + resultCount + ', ' + isMarketOrder + ')">Edit</button>';
                    }
...
function CancelPendingOrder(Id) {
    ...
    }).html("<span class='ui-icon ui-icon-alert' style='float:left; margin:0 7px 10px 0;'></span><span style='font-size:14px;'>The selected order <b style='color:tomato'> [" + Id + "] </b> will be canceled permanently. Are you sure?</span>");
}
...
function EditPendingOrder(Id, rowNumber, isMktOrder) {
    $("#errorEditOrderQty").html = '';
    $("#errorEditOrderPrice").html = '';
    if (isTherEdit == true) {
        //alert("There is an Item For edit Already");
        $("#traderUIPrompt").showEcxPrompt({ title: "Task Overlap.", message: "There is already an item for edit.", type: "warning" });
        return;
    }
    ...
    j.ajax({
        ...
        success: function (data) {
            ...
            $.each(data, function () {
                ...
                html += '<tr><td>Rep ID No:</td><td>' + this.RepIDNo + "</td></tr>";
                html += '<tr><td>Buy/Sell:</td><td>' + this.BuyOrSell + '&nbsp &nbsp' + (this.OrderType == null ? "" : this.OrderType) + '</td></tr>';
                html += '<tr><td>Order Qty:</td><td> Remaining:(' + (remQty == null ? "" : '' + remQty) + ') &nbsp  &nbsp  Original:(' + orginal + ') &nbsp &nbsp' + this.FillTypeName + '</td></tr>';
                html += '<tr><td>Order Price:</td><td>' + (this.Price == null ? "" : this.Price + '&nbsp-&nbsp/') + this.OrderType + '</td></tr>';
                html += '<tr><td>Order Validity:</td><td>' + (this.OrderValidityDate == null ? "" : DayMonthYear + '&nbsp-&nbsp') + this.OrderValidityName + '</td></tr>';
                if (this.IsIF == true) {
                    html += '<tr style="color:Green"><td  >Is IF?&nbsp &nbsp' + (this.IsIF == null ? "False" : '' + this.IsIF) + '</td><td> WHR No. ' + this.WHRNO + '</td></tr>';
...
function SubmitEdit(Id, rowNumber, isMarketOrder) {
    ...
    }).html("<span class='ui-icon ui-icon-alert' style='float:left; margin:0 7px 10px 0;'></span><span style='font-size:14px;'>Are you sure you want to update the selected order [" + Id + "]?</span>");
...
function expandOrderHistoryData(source) {
    ...
    if (source == 1) {
        $("#OrderHistoryTable").css("visibility", "hidden");
    ...
    if ("Expand Change History" === val) {
        ...
        $("#OrderHistoryTable").css("visibility", "visible");
...
function expandOrderIssue(source) {
    ...
    if (source == 1) {
        $("#OrderIssueTable").css("visibility", "hidden");
        $("#OrderDetailIssues").html('Expand');
        $("#OrderIssueData").html('');
        return;
    }
    ...
    if ("Expand Issues" === val) {
        var orderId = $("#OrderdetailOrderId").html().trim();
        $("#OrderIssueTable").css("visibility", "visible");
