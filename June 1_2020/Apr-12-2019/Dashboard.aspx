    ...
    <div class="row-fluid" id="onlineMarketWatch">
        <div id="marketWatch_Tab">
            <section class="row-fluid">
                <section class="span6" style="float: left; -moz-border-radius: 5px; -webkit-border-radius: 5px; border-radius: 5px; text-wrap: normal">
                    <ul>
                ...
                <section class="span6" style="float: left; height: 36px; padding-top: 10px; font-size: 12px; -moz-border-radius: 5px; -webkit-border-radius: 5px; border-radius: 5px;">
...
<asp:Content ID="Content3" ContentPlaceHolderID="rightMasterContent" runat="server">
    <style>
        ...
        .notification {
            ...
            border: thin solid #4eb6fa;
            background-color: #CAF6A4;
            color: #89ac2e;
        }
    </style>
    <script src="Scripts/OrderFormValidation.js"></script>
    <script src="Scripts/MultipleOrderFormValidation.js"></script>
    ...
    <script>
        $().ready(function () {
            ...
            //    if (!window.localStorage.DashboardOpenCount || Number(window.localStorage.DashboardOpenCount) == 0)
            //        window.localStorage.DashboardOpenCount = 1;
            //    else {
            //        alert("Dashboard is already opened.")
            //        //$("<div></div>").showEcxPrompt({ title: "Duplicate Page", message: "Dashboard is already opened.", type: "warning" });
            //        var count = Number(window.localStorage.getItem('DashboardOpenCount'));
            //        window.localStorage.setItem('DashboardOpenCount', count + 1);
            //        //alert('Dashboard is already opened.');
            //        window.close();
            //    }
            //} else {//use cookie
            //    var cnt = getCookieValue('DashboardOpenCount');
            //    if (cnt != 0) {
            //        alert('Dashboard is already opened.');
            //        window.close();
            //    } else {
            //        document.cookie = 'DashboardOpenCount=1;';
            //    }
            //}

            //$(window).on('unload', function (e) {
            //    if (window.localStorage) {
            //        if (window.localStorage.DashboardOpenCount) {
            //            //window.localStorage.removeItem('DashboardOpenCount');
            //            var count = Number(window.localStorage.getItem('DashboardOpenCount'));
            //            if (count > 0)
            //                window.localStorage.setItem('DashboardOpenCount', count - 1)
            //        }
            //    } else {//try to use cookie
            //        delete_cookie('DashboardOpenCount');
            //    }
            //});
            ...
            //    if (window.localStorage) {
            //        if (!window.localStorage.DashboardOpenCount) {
            //            window.localStorage.setItem('DashboardOpenCount', 1);
            //        } else {
            //            var count = Number(window.localStorage.getItem('DashboardOpenCount'));
            //            window.localStorage.setItem('DashboardOpenCount', count + 1)
            //        }
